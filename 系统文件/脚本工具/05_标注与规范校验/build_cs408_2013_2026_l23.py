#!/usr/bin/env python3
"""Reproducible offline CS408 2013–2026 L2/L3 snapshot from reviewed local evidence.
...
VERSION = 'cs408-2013-2026-l23-builder-v1.0.0'
REVIEW_DIR = Path(__file__).with_name('cs408_2013_2026_review_v1')
L1_PATH = '系统文件/题目标注/L1/question_annotations_l1_v1.json'
TOOL_DIR = '系统文件/脚本工具/05_标注与规范校验'
SCHEMA_DIR = '系统文件/数据规范'
OUTPUT_DIR = '系统文件/题目标注/408_2013_2026_L3'
BUNDLE = 'question_annotations_cs408_2013_2026_l23_v1.json'
...
def in_scope(record):
    ...
    return (fact['exam_track']=='CS408' and fact['source_file'].startswith('核心真题/408/')
            and fact['year'] in tuple(range(2013,2027)))
...
def read_reviews(folder):
    rows = {}
    require({p.stem for p in folder.glob('*.psv')}=={str(y) for y in range(2013,2027)}, 'Review years must be 2013–2026')
    ...
    require(len(rows) == 658, f'Expected 658 reviewed questions, found {len(rows)}')
    return rows
...
def dependency_relations(qid,row,rows):
    relations=[]
    for edge in split_items(row['dependencies']):
        a,b=edge.split('>');target=int(b)
        require(1<=target<=len(row['tasks'].split('/')),f'Invalid destination task {qid}:{edge}')
        if a.startswith('@'):
            match=re.fullmatch(r'@(\d{2})([CA])(\d{2})#T(\d{2})',a)
            require(match is not None,f'Invalid cross-question task {qid}:{edge}')
            yy,typ,num,index=match.groups();source_q=f'408-{yy}-{typ}-T{num}';source_i=int(index)
            require(source_q in rows and source_q!=qid,f'Unknown cross-question source {qid}:{edge}')
        else:
            source_q=qid;source_i=int(a)
            require(source_i<target,f'Invalid local direction {qid}:{edge}')
        require(1<=source_i<=len(rows[source_q]['tasks'].split('/')),f'Unknown source task {qid}:{edge}')
        relations.append({'from_question_id':f'{source_q}#T{source_i:02d}',
            'to_question_id':f'{qid}#T{target:02d}',
            'description':'本地解法中后项直接使用前项所得对象或结果；共用题干条件另列context_sources，不视为结果依赖。任务引用不是评分原子。',
            'status':'verified'})
    require(len({(v['from_question_id'],v['to_question_id']) for v in relations})==len(relations),f'Duplicate dependency {qid}')
    return relations
...
def source_inventory(project, review_dir):
    ...
    live_files = {p.relative_to(project).as_posix() for p in (project/'核心真题/408').glob('*.md') if re.match(r'(201[3-9]|202[0-6])年',p.name)}
    require(live_files == set(lock['source_files']), 'CS408 2013–2026 source files were added or removed')
...
def scoring_for(qid, record, block, local_tasks, main, secondary, ref, control):
    ...
    total_claims=[{'points':v['points'],'source_anchor':source_anchor(v['anchor'])} for v in spec.get('explicit_total_claims',[])]
    if mode=='partition':
        require(all(v['points']==total for v in total_claims),f'{qid}: contradictory explicit total')
        ...
        if conflict:
            require(documented and (sum(x['points'] for x in documented)!=total or any(v['points']!=total for v in total_claims)),f'{qid}: conflicting-total mode requires actual conflict')
        unit_id=qid+':S0'
        units=[{'unit_id':unit_id,'points':None if conflict else total,
            ...
            'description':'整题单元；原分值或正文满分与分项口径冲突，评分总分留空。' if conflict else '整题评分单元；原文未提供可完整采用的独立分值拆分。',
    ...
    for i,item in enumerate(spec.get('rules',[]),1):
        ...
        limit=max([total,sum(v['points'] for v in documented)]+[v['points'] for v in total_claims]) if mode=='conflicting_total' else total
        require(item['points'] is None or 0<=item['points']<=limit,f'{qid}: rule points exceed documented scoring bounds')
    ...
    return units,attribution,evidence,{'question_id':qid,'mode':mode,'raw_total_points':total,
        ...
        'documented_subpoints':documented,'conditional_rules':rules,'explicit_total_claims':total_claims,'notes':spec.get('notes',[]),
...
def generate(project,review_dir=REVIEW_DIR):
    ...
    require(set(rows)==set(base), 'Reviewed ids do not exactly cover canonical CS408 2013–2026')
    ...
    for q,refs in policy.get('context_only_references',{}).items():
        require(q in rows and len(refs)==len(set(refs)) and all(v in rows and v!=q for v in refs),'Invalid shared context references')
    ...
    for qid in sorted(rows):
        ...
        l3['inter_question_dependency']['relations']=dependency_relations(qid,row,rows)
        if qid in clusters:
            c=clusters[qid]
        ...
        field_evidence['l3.isomorphic_relation']['basis']='；'.join([policy['empty_policy'],clusters[qid]['scope'] if qid in clusters else '未在本次408的2013—2026年范围确认可填写的匹配关系。'])
        ...
        evidence.append({'question_id':qid,'source':block['evidence'],
                         'review_source':{'file':f'{TOOL_DIR}/cs408_2013_2026_review_v1/{row["review_file"]}','line':row['review_line']},
                         ...
                         'context_sources':[{'question_id':v,'relationship':'shared_statement_context_not_result_dependency','source':blocks[v]['evidence']} for v in policy.get('context_only_references',{}).get(qid,[])],
    ...
    reg={'schema_version':'cs408-label-registry-v1.0.0','scope':'本次408的2013—2026年标注中出现的可引用词条；未冻结全库知识图或能力认证。',
    ...
    report={'schema_version':'cs408-2013-2026-l23-coverage-v1.0.0','builder_version':VERSION,'record_count':658,'inventory':inventory,
    ...
    lines=['# 408的2013—2026年 L2—L3 执行与覆盖报告','',f"完成本地658题可判断标签；{report['human_required_question_count']}题存在受源文冲突影响而留空的字段。",'',
    ...
    lines += ['', '具体冲突与普通质量注记见 coverage_report.json；质量注记不等于标签省略。', '',
        f"共 {report['score_unit_count']} 个评分单元；{report['explicit_partition_question_count']} 题使用 {report['explicit_partition_unit_count']} 个完整一致分项；{len(report['unresolved_total_question_ids'])} 题有效总分留空。",
        f"保存 {report['documented_subpoint_count']} 项明确分值证据、{report['conditional_rule_count']} 条非累加条件规则。{len(tasks)} 个任务引用不是额外题号或评分原子。",
        f"登记 {report['alternative_method_count']} 个备选方法、{report['dependency_count']} 条直接结果依赖、{len(policy['clusters'])} 个同构簇，覆盖 {len(clusters)} 题。",
        '共用前题图表或定义单列 context_sources；仅真正调用前题结果才建立跨题依赖。', '',
        *report['limitations'], '', '完整检查见 validation_report.json 与 run_manifest.json。']
...
def validate_data(project,schema_dir,output,review_dir=REVIEW_DIR):
    ...
    if len(score_byid)!=658 or len(score_records)!=658 or set(score_byid)!=set(base):errors.append('Scoring evidence coverage differs')
    if len(byid)!=658 or len(records)!=658 or bundle['record_count']!=658: errors.append('CS408 2013–2026 record coverage must be exactly 658 unique ids')
    if set(byid)!=set(base): errors.append('Canonical CS408 2013–2026 scope differs')
    if len(evidence)!=658 or {e['question_id'] for e in evidence}!=set(byid):errors.append('Evidence coverage differs')
    ...
    evidence_by_q={v['question_id']:v for v in evidence}
    for q in byid:
        if q not in evidence_by_q or q not in reviewed:
            errors.append(q+': missing canonical evidence');continue
        actual=evidence_by_q[q].get('context_sources',[])
        expected=[{'question_id':v,'relationship':'shared_statement_context_not_result_dependency','source':blocks[v]['evidence']} for v in policy.get('context_only_references',{}).get(q,[])]
        if actual!=expected:errors.append(q+': shared context evidence differs')
    graph=collections.defaultdict(list)
    for r in records:
        q=r['question_id']
        if q not in reviewed:continue
        actual=r['l3'].get('inter_question_dependency',{}).get('relations',[])
        if actual!=dependency_relations(q,reviewed[q],reviewed):errors.append(q+': dependency differs from reviewed direct use')
        for d in actual:graph[d['from_question_id']].append(d['to_question_id'])
    active=set();done=set()
    def visit(v):
        if v in active:errors.append('Cyclic task dependency: '+v);return
        if v in done:return
        active.add(v)
        for dest in graph.get(v,[]):visit(dest)
        active.remove(v);done.add(v)
    for v in list(graph):visit(v)
    expected_tasks=[]
    ...
    for r in records:
        ...
        for dep in l3.get('inter_question_dependency',{}).get('relations',[]):
            ...
            if task_byid[b]['question_id']!=q or (task_byid[a]['question_id']==q and task_byid[a]['task_index']>=task_byid[b]['task_index']):errors.append(q+': invalid/cyclic task dependency')
    ...
    expected_payload=generate(project,review_dir)
    for name,expected in expected_payload.items():
        if output.get(name)!=expected:errors.append('Content differs from fixed reviewed inputs: '+name)
    return {'status':'PASS' if not errors else 'FAIL','record_count':len(records),'error_count':len(errors),'errors':errors,
            'checks':['v1.1 JSON schema','658 unique canonical ids in 2013–2026','original L0/L1 exact preservation','reviewed source hashes and local images',
...
def inputs(project,review_dir,schema_dir):
    result={'project:'+L1_PATH:sha_file(project/L1_PATH),'code:build_cs408_2013_2026_l23.py':sha_file(Path(__file__))}
...
def run(project,output,mode='build',review_dir=REVIEW_DIR,schema_dir=None):
    ...
    temp=Path(tempfile.mkdtemp(prefix='.cs408-2013-2026-l23-',dir=output.parent))
    try:
