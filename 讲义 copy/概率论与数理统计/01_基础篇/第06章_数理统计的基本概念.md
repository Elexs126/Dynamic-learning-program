...
$$
\begin{aligned}\operatorname{cov}(X_{i}, \overline{X}) &= \operatorname{cov}(X_{i}, \frac{1}{n} \sum_{i=1}^{n} X_{i}) = \operatorname{cov}(X_{i}, \frac{X_{i}}{n} + \frac{1}{n} \sum_{j=1 \atop j \neq i}^{n} X_{j}) \\&= \operatorname{cov}(X_{i}, \frac{X_{i}}{n}) + \operatorname{cov}(X_{i}, \frac{1}{n} \sum_{j=1 \atop j \neq i}^{n} X_{j}) \\&= \frac{1}{n} \operatorname{cov}(X_{i}, X_{i}) + \operatorname{cov}(X_{i}, \frac{1}{n} \sum_{j=1 \atop j \neq i}^{n} X_{j}) \\&= \frac{1}{n} D(X_{i}) + \operatorname{cov}(X_{i}, \frac{1}{n} \sum_{j=1 \atop j \neq i}^{n} X_{j}),\end{aligned}
$$
...
【解析】因 $X_{1},X_{2},X_{3},X_{4}$ 为来自总体 $N(0,2^{2})$ 的简单随机样本，故 $X_{i} \sim N(0,2^{2}), \quad (i = 1,2,3,4)$ ，且
...
$$
<!-- DIRECTED-REPAIR-FALLBACK:FALLBACK-QA-R2-PROB-BASE-CH06-P113-TSTAT -->
> [!WARNING]
> 原 PDF 第 113 页回退：t统计量(X̄-μ)/(S/√n)分母结构丢失

![原 PDF 第 113 页回退](../../_directed_repair_assets/FALLBACK-QA-R2-PROB-BASE-CH06-P113-TSTAT_pdf_p0113.png)
...
后续更[新析因为多-磨机[研】 $\frac{1}{4} + 0^{2} = \frac{1}{4}, E(S^{2}) = D(X) = 1$ ,所以
