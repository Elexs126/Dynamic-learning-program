...
设n个人中选中自己原来号码的人数为X,则 $X = X_{1} + X_{2} + \cdots + X_{n}$ .由期望的性质有
...
$$
\int_{-\infty}^{+\infty} x^2 \mathrm{e}^{-(x-\mu)^2} \mathrm{d}x = <!-- DIRECTED-REPAIR-FALLBACK:FALLBACK-QA-R2-PROB-BASE-CH04-P85-NORMALIZE -->
> [!WARNING]
> 原 PDF 第 85 页回退：归一化因子中√2应为1/√2

![原 PDF 第 85 页回退](../../_directed_repair_assets/FALLBACK-QA-R2-PROB-BASE-CH04-P85-NORMALIZE_pdf_p0085.png)

\sqrt{\pi} \int_{-\infty}^{+\infty} x^2 \frac{1}{\sqrt{2\pi} \sqrt{2}} \mathrm{e}^{-\frac{(x-\mu)^2}{2\left(\frac{1}{\sqrt{2}}\right)^2}} \mathrm{d}x = \sqrt{\pi} \left[ \left( \frac{1}{\sqrt{2}} \right)^2 + \mu^2 \right] = \left( \frac{1}{2} + \mu^2 \right) \sqrt{\pi}.
$$
...
【例】设随机变量(X,Y)在以点(0,1),(1,0),(1,1)为顶点的三角形区域上服从二维均匀分布，试求随机变079
...
设(X,Y)是二维随机变量，且E(X)和E(Y)都存在，如果 $E\left\{ \left[ X - E(X) \right] \left[ Y - E(Y) \right] \right\}$ 存在，则称其为随机变量 X与 Y的协方差,记作 cov(X,Y),即
...
【例】设随机变量X和Y的相关系数为 $0.5,E(X)=E(Y)=0,E(X^{2})=E(Y^{2})=2,$ 则 $E(X + Y)^{2} =$ 
