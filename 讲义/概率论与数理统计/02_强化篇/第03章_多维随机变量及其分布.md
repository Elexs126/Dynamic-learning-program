...
④若(X，Y)服从二维正态分布，记 $X_{1}=a_{1}X+b_{1}Y,Y_{1}=a_{2}X+b_{2}Y$ ,则 $( X _ { 1 } , Y _ { 1 } )$ 也服从二维正态分布，其$ 中  \begin{vmatrix}
a_{_1} & b_{_1} \\
a_{_2} & b_{_2}
\end{vmatrix} \neq 0.$
...
【例3.8】设(X,Y)的分布函数为 $F(x,y)=A\left(B+\arctan\frac{x}{2}\right)\left(C+\arctan\frac{y}{3}\right),-\infty<x<+\infty,-\infty<y<+\infty.$ 
...
【例3.11】设二维随机变量(X,Y)的概率密度为 $f(x,y)=A\mathrm{e}^{-2x^{2}+2xy-y^{2}},-\infty<x<+\infty,-\infty<y<+\infty$ ,求常
...
综上得,(X,Y)的概率密度为 $f(x,y)=\left\{\begin{aligned}&\frac{9y^{2}}{x},&0<y<x<1,\\&0,& 其他 .\end{aligned}\right.$  
...
【例3.16】设随机变量(X,Y)服从二维正态分布 $N\left(0,0;1,4;-\frac{1}{2}\right)$ ，下列随机变量中服从标准正态分布且与X独立的是( ).$ 压线更新全点 =\left[ 在扭出 \right]$ (B $\frac{\sqrt{5}}{5}(X - Y)$ 
...
【解析】(1)由 $\int_{-\infty}^{+\infty} \int_{-\infty}^{+\infty} f(x,y) \mathrm{d}x \mathrm{d}y = 1$ ,有 $\int_{0}^{1} \mathrm{d}x \int_{0}^{1} A x y \mathrm{d}y = \frac{A}{4}$ ,故 $A = 4 .$ 
...
【<!-- DIRECTED-REPAIR-FALLBACK:FALLBACK-QA-R2-PROB-ADV-CH03-P188-DERIVATION -->
> [!WARNING]
> 原 PDF 第 188 页回退：F(z)等式链漏掉第一项P{X≤z,X>Y}

![原 PDF 第 188 页回退](../../_directed_repair_assets/FALLBACK-QA-R2-PROB-ADV-CH03-P188-DERIVATION_pdf_p0188.png)
