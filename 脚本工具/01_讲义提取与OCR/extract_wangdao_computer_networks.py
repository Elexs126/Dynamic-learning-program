# -*- coding: utf-8 -*-
"""
王道考研《2027计算机网络考研复习指导》全自动高精度提取与结构化 Markdown 转换脚本
计算机408专业课级深度适配：
- DirectML/GPU硬件加速 + 渲染预取流水线 + 逐页增量日志容灾
- 408 计网知识体系结构化 (考纲内容 / 复习提示 / 知识框架 / 疑难点 / 深度点拨 / 考点追踪)
- 计网核心数学计算与时延/信道公式 KaTeX 标准化 ($C = 2W \\log_2 V$, $C = W \\log_2(1+S/N)$, $U = \\frac{T_D}{T_D + RTT}$, $\\le$, $\\ge$)
- IP / 子网掩码 / CIDR / 路由表 / 报文协议段智能结构化
- 选择题 (A/B/C/D) 与网络综合大题题干、答案与深度解析智能结构化
- 彻底过滤页眉页脚与王道宣传水印
"""
...
CODE_KEYWORDS = [
    r"\btypedef\s+struct\b", r"\bstruct\s+[a-zA-Z_0-9]+",
    r"\bsocket\s*\(", r"\bbind\s*\(", r"\blisten\s*\(", r"\baccept\s*\(", r"\bconnect\s*\(",
    r"\bsend\s*\(", r"\brecv\s*\(", r"\bclose\s*\(",
    r"\bvoid\s+[a-zA-Z_0-9]+\s*\(", r"\bint\s+[a-zA-Z_0-9]+\s*\(",
...
def ocr_rendered_page(pno, img, page_height):
    ...
    for item in results:
        ...
        if y0 < page_height * 0.045:
            if "王道" in text or "计算机网络" in text or "复习指导" in text or ("第" in text and "章" in text):
                continue
...
def wrap_cs_math(line):
    ...
    has_math_ops = bool(re.search(r"(=|\\times|\\div|\+|\-|\*|/|\^|_|\\le|\\ge|\\log)", line))
    ...
    # 包装信道容量与香农/奈氏公式 C=2Wlog2V, C=Wlog2(1+S/N)
    line = re.sub(r"\b2\^([a-zA-Z0-9\+\-]+)\b", r"$2^{\1}$", line)
    line = re.sub(r"\b2\^{([^}]+)}\b", r"$2^{\1}$", line)
    line = re.sub(r"\b10\^([a-zA-Z0-9\+\-]+)\b", r"$10^{\1}$", line)
    line = re.sub(r"\b10\^{([^}]+)}\b", r"$10^{\1}$", line)
    ...
    for pno in range(start_p, end_p + 1):
        ...
        for raw_line in raw_lines:
            ...
            if pno == start_p and re.match(r"^第\s*\d+\s*章", line):
                continue
            if pno == start_p and line in ["计算机网络体系结构", "物理层", "数据链路层", "网络层", "传输层", "应用层"]:
                continue
            ...
            # 识别二级节标题 (如 1.1 计算机网络概述)
            if re.match(r"^\*?\d+\.\d+\s+[\u4e00-\u9fa5]", line):
                md_lines.append(f"\n### {wrap_cs_math(line)}\n")
                in_answers_section = False
                continue
                
            # 识别三级小节标题 (如 1.1.1 计算机网络的概念)
            if re.match(r"^\*?\d+\.\d+\.\d+\s+[\u4e00-\u9fa5]", line):
                md_lines.append(f"\n#### {wrap_cs_math(line)}\n")
                continue
                
            # 识别四级标题 (如 1．数据通信, 1. 概念)
            ...
            # 识别选择题题干编号 (如 01．计算机网络最基本的功能是... 或 11．【2012统考真题】...)
...
def main():
    pdf_path = r"d:\考研动态学习项目\教材\408\27王道《计算机网络》高清带书签【公众号：研料库，料最全】.pdf"
    base_out_dir = r"d:\考研动态学习项目\配套讲义\王道2027计算机网络考研复习指导"
    ...
    print(f"《王道2027计算机网络考研复习指导》总目标页数: {len(all_target_pages)}, 已缓存: {len(pages_dict)}, 待处理: {len(missing_pages)}")
    ...
    print("\n正在生成结构化 Markdown 讲义文档与网络协议/计算排版...")
    for cfg in CHAPTER_CONFIG:
    ...
    print("\n全部提取与格式化完成！所有文件已输出至 配套讲义/王道2027计算机网络考研复习指导/")
