# -*- coding: utf-8 -*-
"""
王道考研《2027数据结构考研复习指导》全自动高精度提取与结构化 Markdown 转换脚本
计算机408专业课级深度适配：
- DirectML/GPU硬件加速 + 渲染预取流水线 + 逐页增量日志容灾
- 408 数据结构知识体系结构化 (考纲内容 / 复习提示 / 知识框架 / 疑难点 / 深度点拨 / 考点追踪)
- C/C++ 数据结构定义 (LNode, BiTNode, ArcNode, RBNode, BTree) 与核心算法 (KMP, 遍历, 排序, 图论) 智能代码块化 (```c ... ```)
- 时间/空间复杂度与树形数学公式 KaTeX 标准化 ($O(n \\log_2 n)$, $n_0 = n_2 + 1$, $WPL = \\sum w_i l_i$, $ASL$, $\\le$, $\\ge$)
- 选择题 (A/B/C/D) 与算法设计大题题干、代码与深度解析智能结构化
- 彻底过滤页眉页脚与王道宣传水印
"""
...
def ocr_rendered_page(pno, img, page_height):
    ...
    for item in results:
        ...
        if y0 < page_height * 0.045:
            if "王道" in text or "数据结构" in text or "复习指导" in text or ("第" in text and "章" in text):
                continue
...
def main():
    pdf_path = r"d:\考研动态学习项目\教材\408\27王道《数据结构》高清带书签【公众号：研料库，料最全】(1).pdf"
    base_out_dir = r"d:\考研动态学习项目\配套讲义\王道2027数据结构考研复习指导"
    ...
    print(f"《王道2027数据结构考研复习指导》总目标页数: {len(all_target_pages)}, 已缓存: {len(pages_dict)}, 待处理: {len(missing_pages)}")
    ...
    print("\n正在生成结构化 Markdown 讲义文档与 C/C++ 算法代码块排版...")
    for cfg in CHAPTER_CONFIG:
    ...
    print("\n全部提取与格式化完成！所有文件已输出至 配套讲义/王道2027数据结构考研复习指导/")
